...
def book_exists_in_db(db, title: str, author: str) -> bool:
    """Return True if a book with the same title+author already exists in the DB (case-insensitive)."""
    return (
        db.query(Book)
...
def import_csv(csv_path: Path, dry_run: bool = False) -> None:
    ...
    # Tracks (title_lower, author_lower) pairs added during this run to catch intra-CSV duplicates
        ...
        with open(csv_path, newline="", encoding="utf-8-sig") as f:
            ...
            for line_num, row in enumerate(reader, start=2):  # 1-indexed, row 1 is header
                ...
                if not title or not author:
                    print(f"  [LINE {line_num}] SKIP — missing title or author: {row}")
                    skipped_missing += 1
                    continue

                key = (title.lower(), author.lower())
                if key in seen_this_run or book_exists_in_db(db, title, author):
